...
function publicMessageUser(u){return u}
function applyHiddenMonitorModeUI(){
...
function renderAll(){renderRooms();renderEntryRoomChoices();renderUsers();renderMessages();renderHeader();renderStage();updateStaffVisibilityUI();applyHiddenMonitorModeUI()}
...
function bind(){
 ...
 if($('#soundBtn')) $('#soundBtn').onclick=()=>{document.body.classList.toggle('muted');const btn=$('#soundBtn');const muted=document.body.classList.contains('muted');const icon=btn?.querySelector('.roomBtnIcon');const label=btn?.querySelector('.roomBtnText');if(icon&&label){icon.textContent=muted?'🔇':'🔊';label.textContent='صوت الغرفة';}else if(btn){btn.textContent=muted?'🔇 صوت الغرفة':'🔊 صوت الغرفة';}toast('تحكم صوت تجريبي')}; $('#radioBtn').onclick=toggleRadioListener;
