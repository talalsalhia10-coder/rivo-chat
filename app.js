...
async function ensureRadioYoutubePlayer(videoId){
 ...
 return await new Promise((resolve,reject)=>{
  let settled=false;
  radioYoutubePlayer=new YT.Player('radioYoutubePlayer',{videoId,playerVars:{controls:1,playsinline:1,rel:0,modestbranding:1,autoplay:0},events:{
...
async function syncRadioForRoom(){
 ...
 if(!(await prepareRadioSource())){renderRadioUI();return}
 if(['youtube','video'].includes(radioType())&&!broadcast.videoClosed)showRadioVideoWindow(false);
